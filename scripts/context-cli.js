...
import { createLogger } from '../src/logging/log.js';
...
program
    ...
    try {
        ...
        const sectionFiles = files.filter(f => f.endsWith('.md'));
        console.log('✅ Sections 目录: ' + sectionFiles.length + ' 个文件');
        if (sectionFiles.length === 0) {
            warnings.push('sections 目录为空');
        }
    }
    catch {
        errors.push('sections 目录不存在');
    }
    // 检查 skills 目录
    const skillsDir = path.resolve(process.cwd(), 'skills');
    try {
    ...
    // 检查系统提示词
    const systemPromptPath = path.resolve(process.cwd(), 'SYSTEM_PROMPT.md');
    try {
        await fs.access(systemPromptPath);
        const content = await fs.readFile(systemPromptPath, 'utf-8');
        console.log('✅ SYSTEM_PROMPT.md: ' + content.length + ' 字符');
    }
    catch {
        errors.push('SYSTEM_PROMPT.md 不存在');
    }
    // 检查配置文件
