...
class DesignStyleSearcher {
     ...
     */
    searchStyles(query, maxResults = 3) {
        if (this.styles.length === 0) {
            return [];
        }
        // Build documents for BM25
        const documents = this.styles.map((style, index) => ({
        ...
        return results.map(result => ({
            style: this.styles[result.score],
            score: result.score,
...
class ColorPaletteSearcher {
     ...
     */
    searchPalettes(query, productType, maxResults = 3) {
        ...
        if (productType) {
            ...
            filteredPalettes = this.palettes.filter((p) => p.category === category);
        }
        // Build documents for BM25
        const documents = filteredPalettes.map((palette, index) => ({
...
class TypographySearcher {
     ...
     */
    searchTypography(query, productType, maxResults = 3) {
        ...
        if (productType) {
            ...
            filteredPairs = this.pairs.filter((p) => p.category === category);
        }
        // Build documents for BM25
        const documents = filteredPairs.map((pair, index) => ({
...
async function main() {
    ...
    if (args.length === 0) {
        ...
        console.log('  node bm25-search.ts recommend <product-type>');
        console.log('\nExamples:');
    ...
    if (command === 'search') {
        if (searchType === 'styles') {
            ...
            const productTypeIdx = args.indexOf('--product-type');
            const productType = productTypeIdx >= 0 ? args[productTypeIdx + 1] : undefined;
...
export { BM25Search, DesignStyleSearcher, ColorPaletteSearcher, TypographySearcher, RecommendationEngine };
//# sourceMappingURL=bm25-search.js.map