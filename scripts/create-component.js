 ...
 */
function parsePropsDefinition(typesDir, typeName) {
    ...
    try {
        ...
        console.error(`✗ Failed to parse props file: ${error}`);
        return getDefaultProps(typeName);
    }
}
/**
 ...
 */
function generateReactComponent(config, props) {
    const { name, description } = config;
    const componentName = name;
    const propsInterface = name + 'Props';
    // Generate props interface
    const propsInterfaceCode = `export interface ${propsInterface} {\n` +
        props.map(p => `  ${p.name}${p.required ? '' : '?'}: ${p.type}${p.defaultValue ? ` // ${p.defaultValue}` : ''}`).join('\n') +
        '\n}';
    // Generate component
 ...
 */
function generateReactNativeComponent(config, props) {
    const { name, description } = config;
    const componentName = name;
    const propsInterface = name + 'Props';
    // Generate props interface
 ...
 */
function generateVueComponent(config, props) {
    const { name, description } = config;
    const componentName = name;
    // Generate props
 ...
 */
function generateSwiftUIComponent(config, props) {
    const { name, description } = config;
    const componentName = name;
 ...
 */
function getTypeScriptToVueType(tsType) {
    const typeMap = {
        'string': 'String',
        'number': 'Number',
        'boolean': 'Boolean',
 ...
 */
function getTypeScriptToSwiftType(tsType) {
    const typeMap = {
        'string': 'String',
        'number': 'Double',
        'boolean': 'Bool',
 ...
 */
function generateIndexFile(componentName, platforms) {
    const exports = [];
